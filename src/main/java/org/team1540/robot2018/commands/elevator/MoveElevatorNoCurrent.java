 ...
 */
public class MoveElevatorNoCurrent extends Command {
  private double target;
  private Timer currentTimer = new Timer();

  public MoveElevatorNoCurrent(double target) {
    this.target = target;
  ...
  protected boolean isFinished() {
    // SmartDashboard.putNumber("[Elevator] Error", Math.abs(Robot.elevator.getError() - target));
    return
        Robot.elevator.getError() < Tuning.elevatorTolerance &&
